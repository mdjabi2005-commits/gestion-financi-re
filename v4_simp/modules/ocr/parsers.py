...
def move_ticket_to_sorted(
    ticket_path: str, 
    categorie: str, 
    sous_categorie: str,
    date_ticket: str = None,
    montant: float = None,
    description: str = None
) -> None:
    """
    Move a ticket to the sorted directory with category/subcategory structure.
    Renames the file with a standardized format for easy retrieval.

    Format: {date}_{categorie}_{montant}_{description}_{original}.ext
    Example: 20241207_Alimentation_45.50_Carrefour_ticket123.jpg

    Args:
        ticket_path: Path to the ticket file
        categorie: Category name
        sous_categorie: Subcategory name
        date_ticket: Transaction date (YYYY-MM-DD format)
    ...
    original_name = os.path.basename(ticket_path)
    name, ext = os.path.splitext(original_name)
    
    # Build new standardized filename
    parts = []
    
    # Add date (format: YYYYMMDD)
    if date_ticket:
        try:
            # Convert YYYY-MM-DD to YYYYMMDD
            date_str = date_ticket.replace("-", "")
            parts.append(date_str)
        except:
            pass
    
    # Add category
    parts.append(categorie.strip().replace(" ", "_"))
    
    # Add amount
    if montant:
        parts.append(f"{montant:.2f}".replace(".", "_"))
    
    # Add description if available (sanitize for filename)
    if description and description.strip():
        # Keep only alphanumeric and basic chars, limit length
        desc_clean = "".join(c for c in description if c.isalnum() or c in " -_")
        desc_clean = desc_clean.strip().replace(" ", "_")[:30]
        if desc_clean:
            parts.append(desc_clean)
    
    # Build new filename
    if parts:
        new_base_name = "_".join(parts) + ext
    else:
        new_base_name = original_name
    
    dest_path = os.path.join(souscat_dir, new_base_name)

    # If a file with the same name exists, create a unique name
    if os.path.exists(dest_path):
        counter = 1
        while os.path.exists(dest_path):
            new_name = f"{os.path.splitext(new_base_name)[0]}_{counter}{ext}"
            dest_path = os.path.join(souscat_dir, new_name)
            counter += 1

    shutil.move(ticket_path, dest_path)
    logger.info(f"Ticket moved and renamed to: {dest_path}")
