...
def fractal_navigation(
    ...
    """
    Render interactive D3.js tree navigation.
    
    Displays hierarchical tree with all nodes visible and connected branches.
    ...
    Args:
        ...
        show_canvas: Whether to display the tree (default: True)
    """
    ...
    if show_canvas:
        ...
        # Read the D3 tree HTML
        html_path = os.path.join(os.path.dirname(__file__), 'd3_tree.html')
        with open(html_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
        
        # Inject the hierarchy data
        hierarchy_json = json.dumps(hierarchy)
        html_with_data = html_content.replace(
            "window.parent.postMessage({ type: 'request-data' }, '*');",
            f"hierarchyData = {hierarchy_json}; renderTree(hierarchyData);"
        )
        
        # Render with components.html - FULL HEIGHT
        components.html(html_with_data, height=1200, scrolling=True)


# OLD FUNCTIONS BELOW - KEPT FOR COMPATIBILITY BUT NOT USED
...
def _get_category_color(label: str) -> str:
    """Get color for category label - GREEN for revenues, RED for expenses."""
    
    # Revenue categories - ALL GREEN
    revenue_categories = {
        'Revenus', 'Salaire', 'Freelance', 'Investissement', 
        'Dividende', 'Intérêt', 'Bonus', 'Autre'
    }
    
    # Expense categories - ALL RED  
    expense_categories = {
        'Dépenses', 'Alimentation', 'Supermarché', 'Restaurant', 'Boulangerie',
        'Transport', 'Autoroute', 'Essence', 'Stationnement',
        'Logement', 'Loyer', 'Santé', 'Loisirs',
        'Factures', 'Abonnement', 'Vêtements', 'Education', 'Uca',
        'Banque', 'Assurance', 'Divers'
    }
    
    if label in revenue_categories:
        return '#10b981'  # Green 🟢
    elif label in expense_categories:
        return '#ef4444'  # Red 🔴
    else:
        # Default: check if it looks like revenue or expense
        if any(keyword in label.lower() for keyword in ['salaire', 'revenu', 'freelance', 'invest']):
            return '#10b981'  # Green
        else:
            return '#ef4444'  # Red
