...
def interface_voir_transactions() -> None:
    ...
    with col_tree:
        ...
        # Render custom Plotly component with click detection
        tree_result = dynamic_tree(
            hierarchy=hierarchy,
    ...
    with col2:
        if st.button("🔄 Réinitialiser filtres", use_container_width=True):
            # Réinitialiser calendrier
            if "cal_transactions_selected_date" in st.session_state:
                st.session_state.cal_transactions_selected_date = None
            if "cal_transactions_date_range" in st.session_state:
                del st.session_state.cal_transactions_date_range
            
            # Réinitialiser les sélections du composant tree (vider le Set)
            # Le JavaScript écoutera ce changement et réinitialisera les couleurs
            if 'tree_transactions_reset' not in st.session_state:
                st.session_state.tree_transactions_reset = 0
