...
def interface_ajouter_revenu() -> None:
    """
    Main interface for adding revenues.

    Supports three modes:
    - Scanner depuis le dossier (PDF scanning with OCR)
    - Ajouter manuellement (manual entry)
    - Revenu récurrent (recurring revenue)

    Features:
    - Uber tax auto-calculation
    - PDF parsing for paystubs
    - OCR logging for performance tracking
    ...
    if mode == "Scanner depuis le dossier":
        ...
        if submit_btn:
            ...
            # Traitement Uber pour tous les revenus récurrents
