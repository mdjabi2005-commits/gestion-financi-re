...
def load_transactions():
    """Charge toutes les transactions depuis la base SQLite."""
...
def insert_transaction(transaction):
    """Insertion simple avec validation V2"""
    errors = validate_transaction_data(transaction)
    if errors:
        st.error(f"❌ Transaction invalide: {', '.join(errors)}")
        return
    
    # Traitement Uber si revenu
    ...
    """, (
        ...
        transaction.get("description", ''),
        safe_convert(transaction['montant']),
        safe_date_convert(transaction["date"]).isoformat(),
...
def interface_transactions_unifiee():
    ...
    if valider:
        if not cat or montant <= 0:
            ...
            st.success(f"✅ Transaction ajoutée ({type_tr}) : {cat} — {transaction_data['montant']:.2f} €")
...
def interface_gerer_recurrences():
    ...
    with tab1:
        conn = get_db_connection()
        df = pd.read_sql_query("SELECT * FROM transactions WHERE source='récurrente_auto' ORDER BY date ASC", conn)
        conn.close()
...
def interface_gerer_transactions():
    ...
    conn = get_db_connection()
    df_all = pd.read_sql_query("SELECT * FROM transactions ORDER BY date ASC", conn)
    conn.close()
    ...
    # 🔥 V2: CONVERSIONS SÉCURISÉES
    df_all['montant'] = df_all['montant'].apply(lambda x: safe_convert(x, float, 0.0))
    df_all['date'] = df_all['date'].apply(lambda x: safe_date_convert(x))
...
def interface_voir_transactions():
    ...
    # 🔥 V2: CONVERSIONS SÉCURISÉES - CONVERSION DES MONTANTS EN FLOAT
    df["montant"] = df["montant"].apply(lambda x: safe_convert(x, float, 0.0))
    ...
    if limit_display and len(df_filtered) > nb_lignes:
        ...
        st.info(f"📋 Affichage des {nb_lignes} dernières transactions sur {len(df_filtered)} trouvées")
    else:
...
def interface_toutes_les_transactions():
    st.subheader("📊 Toutes les transactions V2")
